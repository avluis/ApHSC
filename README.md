...
# Goals:
	...
	Enable/Disable timer when (driver) button is held				[TESTING]
	Implement Auto Startup											[TESTING]
	Enable/Disable Auto Startup when (passenger) button is held		[TESTING]
	Implement a status indicator (Blink)							[TESTING]
	Implement different blink patters								[TESTING]
	Save heat level when (passenger) button is held					[RESEARCHING]
	Startup on saved heat level										[RESEARCHING]
	Remove delay function from Blink method							[RESEARCHING]
